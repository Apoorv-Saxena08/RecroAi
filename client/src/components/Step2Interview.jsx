...
function Step2Interview({ interviewData, onFinish }) {
  ...
  useEffect(() => {
    hasFeedbackRef.current = !!feedback;
  }, [feedback]);
  ...
  if (!isIntroPhase && currentQuestion) {
    setTimeLeft(currentQuestion.timeLimit || 150);
  }
}, [currentIndex]);


  // Request mic permission on mount (fallback if not already granted)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    recognition.onerror = (event) => {
      if (event.error === "not-allowed") {
        console.warn("Microphone permission denied. Enable mic access for voice input.");
  ...
  const submitAnswer = async () => {
    ...
    const fullAnswer = (answer + (interimTranscript ? " " + interimTranscript : "")).trim();
    setInterimTranscript("");
    if (interimTranscript) setAnswer(fullAnswer);
    ...
    <div className='min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-100 flex items-center justify-center p-4 sm:p-6'>
      <div className='w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          ...
          <textarea
            placeholder="Type or speak your answer here... (speech appears live when mic is on)"
            onChange={(e) => {
              setInterimTranscript("");
